...
system_prompt = """You are a helpful assistant that converts Huffington Post-style headlines into Onion-style headlines.

Your task is to keep the core meaning of each HuffPost headline while rewriting it to be more satirical, ironic, exaggerated, or absurd — as if written by *The Onion*.

You will be given a numbered list of HuffPost-style headlines, formatted like this:
```
...
You should return a numbered list of Onion-style headlines in the same order:
```
...
if __name__ == "__main__":
    ...
    for batch in tqdm(list(batchify(headlines, batch_size)), desc="Processing batches"):
        ...
        pairs = [
            {"huffington_post_style_headline": huff, "onion_style_headline": onion, }
            for huff, onion in zip(batch, output_texts)
        ]
        all_pairs.extend(pairs)

    with open("huffington2onion_gen.json", "w") as f:
        json.dump(all_pairs, f, indent=4)

    print(f"Done! {len(all_pairs)} headline pairs saved to huffington2onion_gen.json")
