...
model = AutoModelForCausalLM.from_pretrained("fine_tuned_llama2-sarcastic_to_original")
tokenizer = AutoTokenizer.from_pretrained("fine_tuned_llama2-sarcastic_to_original")
...
def generate_sarcastic_headline(sarcastic_headline, max_new_tokens=50):
    prompt = (
...
input_text = "Community Theater Attempts Shakespeare—Again"
print(f"Sarcastic: {input_text}")
for i in range(5):
    sarcastic_headline = generate_sarcastic_headline(input_text)
    print(f"Original: {sarcastic_headline}")
exit()
